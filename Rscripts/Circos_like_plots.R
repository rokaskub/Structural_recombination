...
# To draw "Large" repeats on the circos plot 
...
## Load and merge all SSA results together
SSA_inf1000 <- fread("../Results/SSA_PE_reads_IS_inf1000.count.csv.gz")[, Sample := NULL]
SSA_inf1000[SampleName == "WT_control"]

SSA_sup1000 <- fread("../Results/SSA_PE_reads_IS_sup1000.count.csv.gz") %>% setnames("Sample", "SampleName")
SSA_sup1000 <- merge(SSA_sup1000, names[, c("SampleName", "Genotype", "Construct", "Suffix", "Name")], by = "SampleName")

unique(SSA_sup1000$SampleName)

Repeats_inf1000 <- fread("../Results/Repeats_PE_reads_IS_inf1000.count.csv.gz")
Repeats_sup1000 <- fread("../Results/Repeats_PE_reads_IS_sup1000.count.csv.gz")

Repeats_inf1000[SampleName == "WT_control"]
Repeats_sup1000[SampleName == "WT_control"]
...
tmp <- merge(sup1000, inf1000, 
             by.x = c("SampleName", "Name", "Genotype", "Construct", "Suffix", "read2.repeat_id"), 
             by.y = c("SampleName", "Name", "Genotype", "Construct", "Suffix", "repeat_id"), 
             all.x = T)
setnames(tmp, "N_inf1000", "read2.normal")

tmp <- merge(tmp, inf1000, 
             by.x = c("SampleName", "Name", "Genotype", "Construct", "Suffix", "read1.repeat_id"), 
             by.y = c("SampleName", "Name", "Genotype", "Construct", "Suffix", "repeat_id"), 
             all.x = T)
...
fwrite(res, "../Results/Results_fraction_COPY.csv")
unique(res$SampleName)

# Load summary.dt
SSA_inf1000_summary <- fread("../Results/SSA_PE_reads_IS_inf1000.count.csv.gz")
SSA_sup1000_summary <- fread("../Results/SSA_PE_reads_IS_sup1000.count.csv.gz")
...
drawLinkCountCircos <- function(dt, size = 367808, min_perc = 0.1) {
  ...
  my_palette <- colorRampPalette(c("lightgrey", "orange", "red"))(max(dt$N))
...
drawLinkCPMCircos(res[SampleName == "WT_control"], min_perc = 0.1)
drawLinkCPMCircos(res[SampleName == "why2-1_TAL_6-2_S3"], min_perc = 0.1)
